...
**Website:** `https://drarpangandhi.com`
...
│   ├── pre-analytical-errors-laboratory.html
│   ├── turnaround-time-laboratory.html
│   ├── great-healthcare-begins-before-treatment.html
│   ├── whats-new-nexora-journey-2026.html    ← What's New post (DRAFT)
│   ├── whats-new-steerx-2026.html
...
```xml
<url>
  <loc>https://drarpangandhi.com/blog/whats-new-august-2026.html</loc>
...
## Google Search Relevance — SEO Rules

Google indexes and ranks pages through three stages: **Crawl → Index → Rank**. Each stage has specific requirements. Missing any one of them means the page may not appear in search results at all.

---

### Stage 1 — Crawling (Can Google find the page?)

- **Sitemap must be complete.** Every page — including all blog posts — must have a `<url>` entry in `sitemap.xml`. Missing pages may never be crawled.
- **robots.txt must not block important pages.** The current `robots.txt` should allow Googlebot to access all HTML pages and images.
- **No login walls or redirects.** Every page must be publicly accessible without authentication.
- **Submit sitemap to Google Search Console** at `https://search.google.com/search-console`. Go to Sitemaps → enter `https://drarpangandhi.com/sitemap.xml` → Submit. Do this once; Google re-reads it automatically.
- **Server errors block indexing.** If a page returns a 404 or 500 error, Google skips it. Always verify new pages load correctly before submitting the sitemap.

---

### Stage 2 — Indexing (Will Google read and store the page?)

Every page must have these four elements — without them Google may not index it:

**Unique `<title>` tag** — 50–60 characters, format: `Topic — Dr. Arpan Gandhi`
```html
<title>Diagnostics Consulting in India — Dr. Arpan Gandhi</title>
```

**Unique `<meta name="description">`** — 150–160 characters, written in plain English, includes the main keyword naturally:
```html
<meta name="description" content="Dr. Arpan Gandhi offers strategic diagnostics consulting for labs and healthcare networks across India, drawing on 29 years of diagnostic medicine leadership." />
```

**Canonical URL tag** — tells Google which URL is the definitive version of this page (prevents duplicate content penalties):
```html
<link rel="canonical" href="https://drarpangandhi.com/consulting.html" />
```
For blog posts: `https://drarpangandhi.com/blog/labs-need-leaders.html`

**One `<h1>` per page** — already enforced in this project. The `<h1>` must match the page topic closely.

**Heading hierarchy** — `<h2>` for section headings, `<h3>` for sub-sections. Never skip levels (no `<h3>` without a preceding `<h2>`).

**Image `alt` text** — every `<img>` must have a descriptive `alt` that includes a relevant keyword where natural:
```html
<img src="images/hero-photo.webp" alt="Dr. Arpan Gandhi, diagnostic medicine consultant, India" />
```

---

### Stage 3 — Ranking (How high does it appear?)

**Mobile-friendliness** — Google uses mobile-first indexing. The site must render correctly on all screen sizes. Already enforced by the responsive CSS work in this project.

**Page speed** — slower pages rank lower. Already using WebP images and `loading="lazy"` on below-fold images. Do not add large uncompressed images.

**Content quality** — thin pages (under ~300 words of real content) rank poorly. Every blog post should be substantive — at least 600–800 words.

**Content freshness** — sites that publish regularly get crawled more often. Publishing a new blog post or What's New update monthly signals an active site to Google.

**Internal linking** — link between related pages inside the site. For example, a blog post about lab leadership should link to `/consulting.html`. This helps Google understand the site structure and passes ranking signals between pages.

**Target keywords for this site** — write content and titles around these phrases:
- `diagnostic medicine consultant India`
- `laboratory medicine expert`
- `healthcare strategist India`
- `NABL quality consultant`
- `diagnostic lab M&A India`
- `ocular pathology India`

---

### Canonical Tag — Add to Every Page

Add this line inside `<head>` on every page, updating the URL per page:

```html
<link rel="canonical" href="https://drarpangandhi.com/PAGE-SLUG.html" />
```
...
| blog/post.html | `https://drarpangandhi.com/blog/post.html` |

---

### SEO Checklist — Every New Page or Blog Post

- [ ] `<title>` is unique, 50–60 chars, includes the main keyword
- [ ] `<meta name="description">` is unique, 150–160 chars
- [ ] `<link rel="canonical">` points to the correct full URL
- [ ] `<h1>` appears exactly once
- [ ] All images have descriptive `alt` text
- [ ] Page URL added to `sitemap.xml`
- [ ] Page is publicly accessible (no errors)
- [ ] Content is at least 600 words (blog posts)
